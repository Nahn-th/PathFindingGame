...
{
    public partial class Form2 : Form, IResettableForm
    {
        //Player
        Image player;
        ...
        private List<Point> FindPathDijkstra(int startX, int startY, int targetX, int targetY)
        {
            ...
            if (startX < 0 || startX >= gridWidth ||
                startY < 0 || startY >= gridHeight ||
                targetX < 0 || targetX >= gridWidth ||
                targetY < 0 || targetY >= gridHeight)
            {
                // Trả về danh sách rỗng nếu chỉ số không hợp lệ
            ...
            if (distance[targetX, targetY] == int.MaxValue)
            {
                // Không có đường đi
                return path; // Trả về danh sách rỗng
            }
            ...
            while (x != startX || y != startY)
            {
                ...
                // Thêm kiểm tra để tránh ngoại lệ
                if (x < 0 || x >= gridWidth || y < 0 || y >= gridHeight)
                {
                    // Lỗi: Chỉ số ngoài phạm vi
                    break;
                }
        ...
        private void MoveNPC()
        {
            ...
            npcGridX = Math.Max(0, Math.Min(npcGridX, gridWidth - 1));
            npcGridY = Math.Max(0, Math.Min(npcGridY, gridHeight - 1));
        ...
        {   // phat hien va cham giua ng va vat the
            if (object1X + object1Width < object2X + 10 || object1X + 10 > object2X + object2Width ||
                object1Y + object1Height < object2Y + 20 || object1Y + 20 > object2Y + object2Height)
            {
